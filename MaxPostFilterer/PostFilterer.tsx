...
export default function PostFilterer({ posts, langCode, onFilteredPosts }: OwnProps) {
  ...
  useEffect(() => {
    ...
    const filtered =  posts.filter((post) =>  visibleTags.filter(visibleTag => post.node.frontmatter.tags.includes(visibleTag)).length !== 0 );
